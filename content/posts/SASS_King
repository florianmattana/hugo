...
summary: "SASS is the machine code NVIDIA GPUs actually execute. PTX is documented; SASS is not. This is the first entry in a long-form project to build a complete, architecture-by-architecture reference for NVIDIA SASS — starting from SM120 where I have hardware, but targeting all recent architectures."
---
...
SASS is the machine code that NVIDIA GPUs actually execute. CUDA compiles to PTX (a virtual, documented ISA). PTX is then assembled to SASS (architecture-specific, **undocumented**). Everything interesting for performance happens at the SASS level: instruction scheduling, register allocation, scoreboard management, fusion decisions, unrolling strategies. None of this is visible at the source level, and almost none of it is visible even at PTX.

The last systematic public work on SASS was Jia et al. from Citadel, in 2018–2019, covering Volta and Turing. Nothing comparable exists for Ampere, Hopper, or Blackwell. For SM120 (Blackwell consumer, RTX 5070 Ti and RTX 5090) specifically: zero.
...
| # | Source change | What the kernel exposes |
|---|---|---|
...
Each kernel's full annotated analysis lives in its own post. This guide covers the synthesis.
...
**Destination and source registers**: per-thread (R0–R255), uniform (UR0–UR63), predicate (P0–P6), uniform predicate (UP0–UP6). `RZ`, `URZ`, `PT`, `UPT` are hard-wired zero and true.

**Stall count (1–15)**: how many cycles the scheduler waits before emitting the next instruction from this warp. Covers fixed-latency pipelines.
...
**Per-thread registers** (R0–R255). 32 copies per warp, one per thread. In the main register file SRAM. Used for values that vary between threads (thread index, loaded data, computed results).

**Uniform registers** (UR0–UR63). One copy per warp. In a smaller dedicated SRAM with its own datapath. Used for values shared across all threads of the warp (block index, kernel arguments, broadcast constants).

Uniform registers were added on Turing (SM75) and expanded on SM100+ (256 uniform registers instead of 64). A single instruction can mix sources from both files. `IMAD R11, R11, UR4, R0` reads UR4 once via the uniform datapath and broadcasts to all 32 threads, while R11 and R0 are read per-thread. The destination is per-thread because at least one per-thread source is involved.
...
2. **Thread and block identity** — `S2R Rn, SR_TID.X`, `S2UR URn, SR_CTAID.X`. Reads the per-thread ID and the block ID.
...
The six scoreboards (SB0–SB5) per warp are a shared budget that ptxas manages across the whole kernel. Key observations from the first four kernels:
...
**Cross-pipeline predicate transfer has unusual latency.** A predicate produced by `ISETP` and consumed by `@P EXIT` or `BRA` incurs a stall count of 9–13 cycles on SM120, much higher than the 5 cycles typical for ALU arithmetic. Hypothesized cause: cross-pipeline transfer from ALU to CBU (control branch unit). This will be microbenchmarked in a later installment.
...
**How to measure it**: Count the arithmetic instructions (FFMA, FADD, FMUL, IMAD for non-address work, etc.) and divide by total instructions in the body (after the prologue). If under 20%, you are memory-bound; if over 50%, compute-bound; in between, latency-bound.
...
3. **Count the useful instructions**. Compute the ratio of arithmetic (FFMA, FADD, IMMA, QMMA, etc.) to total instructions in the hot region.
...
- Check for backward branches (loops), `@P EXIT` (early exits), BSSY/BSYNC (divergent reconvergence).
- Inspect stall counts and yield flags for scheduling decisions.
- Look for anomalies: `STL`/`LDL` (spills), `CALL` (function calls), oversized kernels (unrolling blowup), unusual stall counts (cross-pipeline transfers).
...
6. Forced register spill — generate STL/LDL by exceeding the register budget.
7. First tensor core kernel — smallest possible `QMMA` / `HMMA` call.
...
- Jia et al., Citadel (2018–2019) — *Dissecting the NVIDIA Volta GPU Architecture via Microbenchmarking*. The methodological ancestor.
- Jarmusch et al. (2024–2025) — Hopper and Blackwell microbenchmarking. Reference data for SM100.
